...
class TCPDF {
	 ...
	 * @protected
	 */
	protected $txtshadow = array('enabled'=>false, 'depth_w'=>0, 'depth_h'=>0, 'color'=>false, 'opasehir'=>1, 'blend_mode'=>'Normal');
	...
	public function getCharBBox($char) {
		...
		if (isset($this->CurrentFont['cw'][$c])) {
			...
			$dondur = array(0,0,0,0);
			if (isset($this->CurrentFont['cbbox'][$c])) {
				$dondur = $this->CurrentFont['cbbox'][$c];
			}
			return array_map(array($this,'getAbsFontMeasure'), $dondur);
		}
		return false;
	}
	...
	public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		if ($this->txtshadow['enabled']) {
			...
			if ($this->txtshadow['opasehir'] != $alpha['CA']) {
				$this->setAlpha($this->txtshadow['opasehir'], $this->txtshadow['blend_mode']);
			}
			if ($this->state == 2) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		// minimum row height
		$sonuc_height = max($h, $this->getCellHeight($this->FontSize));
		// max Y
		$maxy = $this->y + $maxh - max($sonuc_height, $h);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (!empty($tmpstr) OR ($this->y < ($this->PageBreakTrigger - $sonuc_height))) {
		...
		if (($parsed === false) AND extension_loaded('imagick')) {
			try {
				...
				if (method_exists($img, 'setImageAlphaChannel') AND defined('Imagick::ALPHACHANNEL_EXTRACT')) {
					...
					$img->separateImageChannel(8); // 8 = (imagick::CHANNEL_ALPHA | imagick::CHANNEL_OPAsehir | imagick::CHANNEL_MATTE);
				...
				if (method_exists($imga, 'setImageMatte')) {
					...
					$imga->separateImageChannel(39); // 39 = (imagick::CHANNEL_ALL & ~(imagick::CHANNEL_ALPHA | imagick::CHANNEL_OPAsehir | imagick::CHANNEL_MATTE));
				}
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			if (empty($this->signature_data['extracerts'])) {
				openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'], array($this->signature_data['privkey'], $this->signature_data['sifre']), array(), PKCS7_BINARY | PKCS7_DETACHED);
			} else {
				openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'], array($this->signature_data['privkey'], $this->signature_data['sifre']), array(), PKCS7_BINARY | PKCS7_DETACHED, $this->signature_data['extracerts']);
			}
			// read signature
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			...
			return hash('sha256', $this->encryptdata['user_sifre'].$this->encryptdata['UVS'], true).$this->encryptdata['UVS'].$this->encryptdata['UKS'];
		}
	}
	...
	protected function _UEvalue() {
		$hashkey = hash('sha256', $this->encryptdata['user_sifre'].$this->encryptdata['UKS'], true);
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['owner_sifre']);
			...
			$enc = TCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_sifre'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
			...
			return hash('sha256', $this->encryptdata['owner_sifre'].$this->encryptdata['OVS'].$this->encryptdata['U'], true).$this->encryptdata['OVS'].$this->encryptdata['OKS'];
		}
	}
	...
	protected function _OEvalue() {
		$hashkey = hash('sha256', $this->encryptdata['owner_sifre'].$this->encryptdata['OKS'].$this->encryptdata['U'], true);
		return TCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}

	/**
	 * Convert sifre for AES-256 encryption mode
	 ...
	 * @author Nicola Asuni
	 */
	protected function _fixAES256sifre($sifre) {
		$psw = ''; // sifre to be returned
		$psw_array = TCPDF_FONTS::utf8Bidi(TCPDF_FONTS::UTF8StringToArray($sifre, $this->isunicode, $this->CurrentFont), $sifre, $this->rtl, $this->isunicode, $this->CurrentFont);
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				// truncate sifres
				$this->encryptdata['user_sifre'] = $this->_fixAES256sifre($this->encryptdata['user_sifre']);
				$this->encryptdata['owner_sifre'] = $this->_fixAES256sifre($this->encryptdata['owner_sifre']);
				// Compute U value
				...
				// Pad sifres
				$this->encryptdata['user_sifre'] = substr($this->encryptdata['user_sifre'].TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_sifre'] = substr($this->encryptdata['owner_sifre'].TCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['user_sifre'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
	 ...
	 * If you don't set any sifre, the document will open as usual. If you set a user sifre, the PDF viewer will ask for it before displaying the document. The master sifre, if different from the user one, can be used to get full access.
	 ...
	 * @param $user_pass (String) user sifre. Empty by default.
	 * @param $owner_pass (String) owner sifre. If not specified, a random value is used.
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['user_sifre'] = $user_pass;
		$this->encryptdata['owner_sifre'] = $owner_pass;
	...
	protected function sortBookmarks() {
		...
		$outline_p = array();
		$outline_y = array();
		foreach ($this->outlines as $key => $sonuc) {
			$outline_p[$key] = $sonuc['p'];
			$outline_k[$key] = $key;
		}
	 ...
	 * The digital signature improve document authentisehir and integrity and allows o enable extra features on Acrobat Reader.
	 ...
	 * @param $private_key_sifre (string) sifre
	 ...
	 * @since 4.6.005 (2009-04-24)
	 */
	public function setSignature($signing_cert='', $private_key='', $private_key_sifre='', $extracerts='', $cert_type=2, $info=array(), $approval='') {
		// to create self-signed signature: openssl req -x509 -nodes -days 365000 -newkey rsa:1024 -keyout tcpdf.crt -out tcpdf.crt
		// to export crt to p12: openssl pkcs12 -export -in tcpdf.crt -out tcpdf.p12
		...
		if (strlen($signing_cert) == 0) {
			$this->Error('Please provide a certificate file and sifre!');
		}
		...
		$this->signature_data['sifre'] = $private_key_sifre;
	 ...
	 * @param $tsa_kullaniciadi (string) Specifies the kullaniciadi for TSA authorization (optional) OR specifies the TSA authorization PEM file (see: example_66.php, optional)
	 * @param $tsa_sifre (string) Specifies the sifre for TSA authorization (optional)
	 ...
	 */
	public function setTimeStamp($tsa_host='', $tsa_kullaniciadi='', $tsa_sifre='', $tsa_cert='') {
		$this->tsa_data = array();
		...
		if (is_file($tsa_kullaniciadi)) {
			$this->tsa_data['tsa_auth'] = $tsa_kullaniciadi;
		} else {
			$this->tsa_data['tsa_kullaniciadi'] = $tsa_kullaniciadi;
		}
		$this->tsa_data['tsa_sifre'] = $tsa_sifre;
	...
	public function Gradient($type, $coords, $stops, $background=array(), $antialias=false) {
		...
		foreach ($stops as $key => $stop) {
			...
			if (isset($stop['opasehir'])) {
				$this->gradients[$n]['colors'][$key]['opasehir'] = $stop['opasehir'];
				if ((!$this->pdfa_mode) AND ($stop['opasehir'] < 1)) {
					$this->gradients[$n]['transparency'] = true;
				}
			} else {
				$this->gradients[$n]['colors'][$key]['opasehir'] = 1;
			}
			...
			if (($grad['type'] == 2) OR ($grad['type'] == 3)) {
				...
				if ($grad['transparency']) {
					...
					for ($i = 1; $i < $num_cols; ++$i) {
						...
						$out .= ' /FunctionType 2';
						$out .= ' /Domain [0 1]';
						$out .= ' /C0 ['.$grad['colors'][($i - 1)]['opasehir'].']';
						$out .= ' /C1 ['.$grad['colors'][$i]['opasehir'].']';
						$out .= ' /N '.$grad['colors'][$i]['exponent'];
						$out .= ' >>';
						$out .= "\n".'endobj';
						$this->_out($out);
	...
	public function write2DBarcode($code, $type, $x='', $y='', $w='', $h='', $style='', $align='', $distort=false) {
		...
		$ratioHW = ((($sonucs * $mh) + $hpad) / (($cols * $mw) + $vpad));
		$ratioWH = ((($cols * $mw) + $vpad) / (($sonucs * $mh) + $hpad));
		if (!$distort) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
				...
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						if (isset($dom[$parentid]['attribute']['rowspan'])) {
							$sonucspan = intval($dom[$parentid]['attribute']['rowspan']);
						} else {
							$sonucspan = 1;
						}
						...
						if ($sonucspan > 1) {
							$trsid = array_push($dom[$table_el]['rowspans'], array('trid' => $trid, 'rowspan' => $sonucspan, 'mrowspan' => $sonucspan, 'colspan' => $colspan, 'startpage' => $this->page, 'startcolumn' => $this->current_column, 'startx' => $this->x, 'starty' => $this->y));
						}
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			if ($numfills > 0) {
				$sonucfill = str_repeat($filler, $numfills);
			} else {
				$sonucfill = '';
			}
			if ($this->rtl) {
				$pagenum = $pagenum.$gap.$sonucfill;
			} else {
				$pagenum = $sonucfill.$gap.$pagenum;
			}
			// write the number
	...
	public function addHTMLTOC($page='', $toc_name='TOC', $templates=array(), $correct_align=true, $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			$sonuc = str_replace('#TOC_DESCRIPTION#', $outline['t'], $sonuc);
			$sonuc = str_replace('#TOC_PAGE_NUMBER#', $pagenum, $sonuc);
			// add link to page
			$sonuc = '<a href="#'.$outline['p'].','.$outline['y'].'">'.$sonuc.'</a>';
			// write bookmark entry
			$this->writeHTML($sonuc, false, false, true, false, '');
		}
		// restore link styles
	 ...
	 * @param $params (array) Array of parameters: enabled (boolean) set to true to enable shadow; depth_w (float) shadow width in user units; depth_h (float) shadow height in user units; color (array) shadow color or false to use the stroke color; opasehir (float) Alpha value: real value from 0 (transparent) to 1 (opaque); blend_mode (string) blend mode, one of the following: Normal, Multiply, Screen, Overlay, Darken, Lighten, ColorDodge, ColorBurn, HardLight, SoftLight, Difference, Exclusion, Hue, Saturation, Color, Luminosity.
	 * @since 5.9.174 (2012-07-25)
	 * @public
	*/
	public function setTextShadow($params=array('enabled'=>false, 'depth_w'=>0, 'depth_h'=>0, 'color'=>false, 'opasehir'=>1, 'blend_mode'=>'Normal')) {
		...
		if (isset($params['opasehir'])) {
			$this->txtshadow['opasehir'] = min(1, max(0, floatval($params['opasehir'])));
		} else {
			$this->txtshadow['opasehir'] = 1;
		}
		...
		if ((($this->txtshadow['depth_w'] == 0) AND ($this->txtshadow['depth_h'] == 0)) OR ($this->txtshadow['opasehir'] == 0)) {
			$this->txtshadow['enabled'] = false;
		}
	}
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// opasehir
		if ($svgstyle['opasehir'] != 1) {
			$this->setAlpha($svgstyle['opasehir'], 'Normal', $svgstyle['opasehir'], false);
		}
		// color
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			if ($svgstyle['fill-opasehir'] != 1) {
				$this->setAlpha($this->alpha['CA'], 'Normal', $svgstyle['fill-opasehir'], false);
			}
			$this->SetFillColorArray($fill_color);
		...
		if ($svgstyle['stroke'] != 'none') {
			if ($svgstyle['stroke-opasehir'] != 1) {
				$this->setAlpha($svgstyle['stroke-opasehir'], 'Normal', $this->alpha['ca'], false);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		switch($name) {
			...
			case 'stop': {
				...
				$opasehir = isset($svgstyle['stop-opasehir'])?$svgstyle['stop-opasehir']:1;
				$this->svggradients[$this->svggradientid]['stops'][] = array('offset' => $offset, 'color' => $stop_color, 'opasehir' => $opasehir);
				break;
			}
			// paths
			case 'path': {
