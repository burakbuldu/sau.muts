...
// -----------------------------------------------------------------------------
				...
				for ($hr = 0; $hr < $params[8]; ++$hr) {
					...
					for ($c = 0; $c < $params[5]; ++$c) {
						...
						if ($r == 0) {
							...
							if ($places[$i] < 2) {
								$grid[$sonuc][$col] = $places[$i];
							} else {
								// codeword ID
								$cw_id = (floor($places[$i] / 10) - 1);
								// codeword BIT mask
								$cw_bit = pow(2, (8 - ($places[$i] % 10)));
								$grid[$sonuc][$col] = (($cw[$cw_id] & $cw_bit) == 0) ? 0 : 1;
							}
							++$i;
						}
					}
	 ...
	 * @protected
	 */
	protected function placeModule($marr, $nrow, $ncol, $sonuc, $col, $chr, $bit) {
		if ($sonuc < 0) {
			$sonuc += $nrow;
			$col += (4 - (($nrow + 4) % 8));
		}
		...
		$marr[(($sonuc * $ncol) + $col)] = ((10 * $chr) + $bit);
		return $marr;
	}
	 ...
	 * @protected
	 */
	protected function placeUtah($marr, $nrow, $ncol, $sonuc, $col, $chr) {
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc-2, $col-2, $chr, 1);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc-2, $col-1, $chr, 2);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc-1, $col-2, $chr, 3);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc-1, $col-1, $chr, 4);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc-1, $col,   $chr, 5);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc,   $col-2, $chr, 6);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc,   $col-1, $chr, 7);
		$marr = $this->placeModule($marr, $nrow, $ncol, $sonuc,   $col,   $chr, 8);
		return $marr;
	}
	...
	protected function getPlacementMap($nrow, $ncol) {
		...
		do {
			...
			if (($sonuc == $nrow) AND ($col == 0)) {
				$marr = $this->placeCornerA($marr, $nrow, $ncol, $chr);
				++$chr;
			}
			if (($sonuc == ($nrow - 2)) AND ($col == 0) AND ($ncol % 4)) {
				$marr = $this->placeCornerB($marr, $nrow, $ncol, $chr);
				++$chr;
			}
			if (($sonuc == ($nrow - 2)) AND ($col == 0) AND (($ncol % 8) == 4)) {
				$marr = $this->placeCornerC($marr, $nrow, $ncol, $chr);
				++$chr;
			}
			if (($sonuc == ($nrow + 4)) AND ($col == 2) AND (!($ncol % 8))) {
				$marr = $this->placeCornerD($marr, $nrow, $ncol, $chr);
				++$chr;
			}
			// sweep upward diagonally, inserting successive characters,
			do {
				if (($sonuc < $nrow) AND ($col >= 0) AND (!$marr[(($sonuc * $ncol) + $col)])) {
					$marr = $this->placeUtah($marr, $nrow, $ncol, $sonuc, $col, $chr);
					++$chr;
				}
				$sonuc -= 2;
				$col += 2;
			} while (($sonuc >= 0) AND ($col < $ncol));
			++$sonuc;
			$col += 3;
			// & then sweep downward diagonally, inserting successive characters,...
			do {
				if (($sonuc >= 0) AND ($col < $ncol) AND (!$marr[(($sonuc * $ncol) + $col)])) {
					$marr = $this->placeUtah($marr, $nrow, $ncol, $sonuc, $col, $chr);
					++$chr;
				}
				$sonuc += 2;
				$col -= 2;
			} while (($sonuc < $nrow) AND ($col >= 0));
			$sonuc += 3;
			++$col;
			// ... until the entire array is scanned
		} while (($sonuc < $nrow) OR ($col < $ncol));
