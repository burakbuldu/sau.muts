...
class PDF417 {
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		if ($numcw > 925) {
			// reached maximum data codeword capasehir
			return false;
		}
		...
		$sonucs = ceil($nce / $cols);
		$size = ($cols * $sonucs);
		// adjust rows
		if (($sonucs < 3) OR ($sonucs > 90)) {
			if ($sonucs < 3) {
				$sonucs = 3;
			} elseif ($sonucs > 90) {
				$sonucs = 90;
			}
			$cols = ceil($size / $sonucs);
			$size = ($cols * $sonucs);
		}
		if ($size > 928) {
			// set dimensions to get maximum capasehir
		...
		$barcode_array['num_rows'] = ($sonucs * ROWHEIGHT) + (2 * QUIETV);
		...
		for ($r = 0; $r < $sonucs; ++$r) {
			...
			switch ($cid) {
				case 0: {
					$L = ((30 * intval($r / 3)) + intval(($sonucs - 1) / 3));
					break;
				}
				case 1: {
					$L = ((30 * intval($r / 3)) + ($ecl * 3) + (($sonucs - 1) % 3));
					break;
				}
				case 2: {
			...
			// left row indicator
			$sonuc .= sprintf('%17b', $this->clusters[$cid][$L]);
			// for each column
			for ($c = 0; $c < $cols; ++$c) {
				$sonuc .= sprintf('%17b', $this->clusters[$cid][$codewords[$k]]);
				++$k;
			}
			switch ($cid) {
				case 0: {
			...
			$arow = preg_split('//', $sonuc, -1, PREG_SPLIT_NO_EMPTY);
