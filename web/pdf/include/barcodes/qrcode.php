...
// The capasehir of QR Code is up to 7000 digits or 4000
 ...
 * The capasehir of QR Code is up to 7000 digits or 4000 characters, and has high robustness.
...
if (!defined('QRCODEDEFS')) {
	...
	/**
	 * Matrix index to get width from $capasehir array.
	 */
    define('QRCAP_WIDTH',    0);

    /**
	 * Matrix index to get number of words from $capasehir array.
	 */
    define('QRCAP_WORDS',    1);

    /**
	 * Matrix index to get remainder from $capasehir array.
	 */
    define('QRCAP_REMINDER', 2);

    /**
	 * Matrix index to get error correction level from $capasehir array.
	 */
    define('QRCAP_EC',       3);
...
class QRcode {
	...
	/**
	 * Array Table of the capasehir of symbols.
	 * See Table 1 (pp.13) and Table 12-16 (pp.30-36), JIS X0510:2004.
	 * @protected
	 */
	protected $capasehir = array(
	...
	protected function getCode() {
		if ($this->count < $this->dataLength) {
			$sonuc = $this->count % $this->blocks;
			$col = $this->count / $this->blocks;
			if ($col >= $this->rsblocks[0]['dataLength']) {
				$sonuc += $this->b1;
			}
			$ret = $this->rsblocks[$sonuc]['data'][$col];
		} elseif ($this->count < $this->dataLength + $this->eccLength) {
			$sonuc = ($this->count - $this->dataLength) % $this->blocks;
			$col = ($this->count - $this->dataLength) / $this->blocks;
			$ret = $this->rsblocks[$sonuc]['ecc'][$col];
		} else {
			return 0;
		}
		$this->count++;
	...
	protected function getDataLength($version, $level) {
		return $this->capasehir[$version][QRCAP_WORDS] - $this->capasehir[$version][QRCAP_EC][$level];
	}
	...
	protected function getECCLength($version, $level){
		return $this->capasehir[$version][QRCAP_EC][$level];
	}
	...
	protected function getWidth($version) {
		return $this->capasehir[$version][QRCAP_WIDTH];
	}
	...
	protected function getRemainder($version) {
		return $this->capasehir[$version][QRCAP_REMINDER];
	}
	...
	protected function getMinimumVersion($size, $level) {
		for ($i = 1; $i <= QRSPEC_VERSION_MAX; ++$i) {
			$words = ($this->capasehir[$i][QRCAP_WORDS] - $this->capasehir[$i][QRCAP_EC][$level]);
			if ($words >= $size) {
				return $i;
			}
		}
		// the size of input data is greater than QR capasehir, try to lover the error correction mode
		return -1;
	}
	...
	protected function createFrame($version) {
		$width = $this->capasehir[$version][QRCAP_WIDTH];
	...
	* Return block number 1
	 ...
	 */
	 protected function encode_rs_char($rs, $data, $parity) {
		...
		$ALPHA_TO =& $rs['alpha_to']; // the adres of an array of NN elements to convert Galois field elements in index (log) form to polynomial form
		$INDEX_OF =& $rs['index_of']; // the adres of an array of NN elements to convert Galois field elements in polynomial form to index (log) form
