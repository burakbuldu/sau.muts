...
class TCPDF_STATIC {
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function _AES($key, $text) {
		// padding (RFC 2898, PKCS #5: sifre-Based Cryptography Specification Version 2.0)
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		// sifre: Specifies whether the field should display asterisks when data is entered in the field.
		if (isset($prop['sifre']) AND ($prop['sifre'] == 'true')) {
			$ff += 1 << 13;
		}
		...
		// kullaniciadi: The user name (short description string) of the field.
		if (isset($prop['kullaniciadi'])) {
			$opt['tu'] = $prop['kullaniciadi'];
		}
	...
	public static function extractCSSproperties($cssdata) {
		...
		foreach ($cssblocks as $block) {
			$selector = $block[0];
			// calculate selector's specifisehir
			$matches = array();
			...
			$specifisehir = $a.$b.$c.$d;
			// add specifisehir to the beginning of the selector
			$cssdata[$specifisehir.' '.$selector] = $block[1];
		}
		// sort selectors alphabetically to account for specifisehir
	...
	public static function getCSSdataArray($dom, $key, $css) {
		...
		foreach($css as $selector => $style) {
			$pos = strpos($selector, ' ');
			// get specifisehir
			$specifisehir = substr($selector, 0, $pos);
			// remove specifisehir
			...
			if (self::isValidCSSSelectorForTag($dom, $key, $selector)) {
				if (!in_array($selector, $selectors)) {
					...
					$cssarray[] = array('k' => $selector, 's' => $specifisehir, 'c' => $style);
					$selectors[] = $selector;
				}
			}
		}
		...
		// order the css array to account for specifisehir
		$cssordered = array();
